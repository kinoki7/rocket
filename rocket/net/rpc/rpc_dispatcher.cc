...
void RpcDispatcher::dispatch(AbstractProtocol::s_ptr request, AbstractProtocol::s_ptr response, TcpConnection* connection) {
    ...
    if(it == m_service_map.end()) {
        ERRORLOG("%s | service name [%s] not found", request->m_req_id, service_name.c_str());
    ...
    if(method == NULL) {
        ERRORLOG("%s | method name [%s] not found in service[%s]", request->m_req_id, method_name.c_str(), service_name.c_str());
    ...
    if(!req_msg->ParseFromString(req_protocol->m_pb_data)) {
        ERRORLOG("%s | deserlialize error, origin message[%s]", request->m_req_id, req_msg->ShortDebugString().c_str());
