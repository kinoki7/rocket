...
void RpcChannel::CallMethod(const google::protobuf::MethodDescriptor* method,
    ...
    std::shared_ptr<rocket::TinyPBProtocol> req_protocol = std::make_shared<TinyPBProtocol>();
    
    RpcController* my_controller = dynamic_cast<RpcController*>(controller);
    if(my_controller == NULL) {
        ERRORLOG("failed callmethod, RpcContoller convert error");
        return;
    }
    ...
    m_client->connect([req_protocol, channel]() mutable{
       ...
       RpcController* my_controller = dynamic_cast<RpcController*>(channel->getController());
        ...
        if(channel->getTcpClient()->getConnectErrorCode() != 0) {
            ...
            ERRORLOG("%s | connect error, errorcode[%d], error info[%s], peer addr[%s]", 
             req_protocol->m_msg_id.c_str(), my_controller->GetErrorCode(),my_controller->GetErrorInfo().c_str(), channel->getTcpClient()->getPeerAddr()->toString().c_str());

            return;
        }
        channel->getTcpClient()->writeMessage(req_protocol, [my_controller, req_protocol, channel](AbstractProtocol::s_ptr) mutable{
            INFOLOG("%s | , send rpc request success, call method_name[%s], peer addr[%s], local addr[%s]", 
            ...
            channel->getTcpClient()->readMessage(req_protocol->m_msg_id, [my_controller, channel](AbstractProtocol::s_ptr msg) mutable{
                ...
                if(rsp_protocol->m_err_code != 0) {
                    ERRORLOG("%s | call rpc method[%s] failed, error code[%d], error info[%s]",rsp_protocol->m_msg_id.c_str(), 
                    rsp_protocol->m_method_name.c_str(), rsp_protocol->m_err_code, rsp_protocol->m_err_info.c_str());
                    my_controller->SetError(rsp_protocol->m_err_code, rsp_protocol->m_err_info);

                    return;
                }

                INFOLOG("%s | call rpc success, call method_name[%s], peer addr[%s], local addr[%s]", 
                 rsp_protocol->m_msg_id.c_str(), rsp_protocol->m_method_name.c_str(), 
                  channel->getTcpClient()->getPeerAddr()->toString().c_str(), channel->getTcpClient()->getLocalAddr()->toString().c_str());
